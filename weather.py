...
# 그래프 설정
        def weather_graph(): # 날씨 그래프
            ...
            for i in range(len(x)): # 그래프 값 표시
                height = y[i]
                plt.text(x[i], height + 0.2, y[i], ha='center')
        ...
        def pre_graph(): # 강수 그래프
            ...
            values = [int(pre_graph_mm[0].text), # 값 설정
                      int(pre_graph_mm[1].text), 
                      int(pre_graph_mm[2].text), 
                      int(pre_graph_mm[3].text), 
                      int(pre_graph_mm[4].text), 
                      int(pre_graph_mm[5].text), 
                      int(pre_graph_mm[6].text), 
                      int(pre_graph_mm[7].text), 
                      int(pre_graph_mm[8].text), 
                      int(pre_graph_mm[9].text), 
                      int(pre_graph_mm[10].text), 
                      int(pre_graph_mm[11].text)]
            ...
            if max(values)==0: # 최대 강수량이 '0'일 때
                plt.ylim(0, 5)
            plt.bar(label, values, color = 'teal')
            
            for i in range(len(label)): # 그래프 값 표시
                height = values[i]
                plt.text(label[i], height + 0.1, info[i], ha='center')
                plt.text(label[i], height - 0.2, values[i], ha='center')
        ...
        def wind_graph(): # 바람 그래프
            ...
            for i in range(len(label)): # 그래프 값 표시
                height = values[i]
                plt.text(label[i], height + 0.1, info[i], ha='center')
                plt.text(label[i], height - 0.3, values[i], ha='center')
        ...
        def humidity_graph(): # 습도 그래프
            ...
            for i in range(len(label)): # 그래프 값 표시
                height = values[i]
                plt.text(label[i], height + 0.5, values[i], ha='center')
